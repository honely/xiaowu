<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:71:"G:\xampp\htdocs\bbb\public/../application/admin\view\district\city.html";i:1541389042;s:70:"G:\xampp\htdocs\bbb\public/../application/admin\view\index\header.html";i:1536287308;s:70:"G:\xampp\htdocs\bbb\public/../application/admin\view\index\footer.html";i:1525742360;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    ...
    <table lay-skin="line" class="layui-table" lay-filter="parse-table-demo" style="padding: 10px;text-align: center;border: 1px;solid-color: #28282c;text-align: left">
        ...
        <tbody>
            <?php if(is_array($city) || $city instanceof \think\Collection || $city instanceof \think\Paginator): $i = 0; $__LIST__ = $city;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                <tr>
                    <td style="padding-left: 50px;"><?php echo $vo['c_id']; ?></td>
                    <td><?php echo $vo['p_name']; ?></td>
                    <td><?php echo $vo['c_name']; ?></td>
                    <td><?php echo $vo['c_code']; ?></td>
                    <td>
                    <button class="layui-btn layui-btn-xs" onclick="editCity(<?php echo $vo['c_id']; ?>)"><i class="layui-icon">&#xe642;</i>编辑</button>
                    <button class="layui-btn layui-btn-danger layui-btn-xs" onclick="delCity(<?php echo $vo['c_id']; ?>)" data-type="test2"><i class="layui-icon">&#xe640;</i>删除</button>
                    </td>
                </tr>
