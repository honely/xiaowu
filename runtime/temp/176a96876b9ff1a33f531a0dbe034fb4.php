<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:74:"G:\xampp\htdocs\bbb\public/../application/decoration\view\index\index.html";i:1544579067;}*/ ?>
<!DOCTYPE html>
<html>
        ...
        <?php if(is_array($houses) || $houses instanceof \think\Collection || $houses instanceof \think\Paginator): $i = 0; $__LIST__ = $houses;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$hous): $mod = ($i % 2 );++$i;?>
            <div class="mui-card">
                ...
                <div class="mui-card-footer">
                    <a class="mui-card-link" href="<?=url('index/details')?>?h_id=<?php echo $hous['h_b_id']; ?>&type=3">房源详情</a>
                    <a class="mui-card-link" href="<?=url('index/timeline')?>?h_id=<?php echo $hous['h_b_id']; ?>&type=3">装修进度</a>
                    <a class="mui-card-link" href="<?=url('index/dailylog')?>?h_id=<?php echo $hous['h_b_id']; ?>&type=3">监理记录</a>
                </div>
            </div>
                ...
                if(data.length<=0){
                    ...
                    for (var i=0;i<data.length;i++) {
                        ...
                        html+=' 签订日期：'+data[i].h_addtime+'<br/>'+
                            ...
                            '        </div>' +
                            '<div class="mui-card-footer">' +
                            '            <a class="mui-card-link" href="<?=url('index/details')?>?h_id='+data[i].h_b_id+'&type=3">房源详情</a>' +
                            '            <a class="mui-card-link" href="<?=url('index/timeline')?>?h_id='+data[i].h_b_id+'&type=3">装修进度</a>' +
                            '            <a class="mui-card-link" href="<?=url('index/dailylog')?>?h_id='+data[i].h_b_id+'&type=3">监理记录</a>' +
                            '        </div></div>';
                    }
                }
                $('#getMore').append(html);
