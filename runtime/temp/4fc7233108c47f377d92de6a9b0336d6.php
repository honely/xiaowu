<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:68:"G:\xampp\htdocs\bbb\public/../application/admin\view\staff\edit.html";i:1542604506;s:70:"G:\xampp\htdocs\bbb\public/../application/admin\view\index\header.html";i:1536287308;s:70:"G:\xampp\htdocs\bbb\public/../application/admin\view\index\footer.html";i:1525742360;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    ...
    <div style="padding: 15px;">
        <form class="layui-form layui-form-pane1" action="<?=url('staff/edit')?>?u_id=<?php echo $admin['u_id']; ?>" method="post">
            ...
            <div class="layui-form-item" pane>
                ...
                <div class="layui-input-block">
                    <input type="radio" name="u_sex" value="1" title="男" <?php if($admin['u_sex'] == 1): ?>checked<?php endif; ?> >
                    <input type="radio" name="u_sex" value="2" title="女" <?php if($admin['u_sex'] == 2): ?>checked<?php endif; ?>>
                </div>
            </div>
            <div class="layui-form-item">
                <label class="layui-form-label"><span style="color: red;">*</span>手机号码</label>
                <div class="layui-input-block">
                    <input type="text" name="u_phone" id="u_phone" onblur="checkPhone()" value="<?php echo $admin['u_phone']; ?>" lay-verify="required|phone" placeholder="请输入手机号码" autocomplete="off" class="layui-input">
                </div>
            </div>
            <div class="layui-form-item">
                <label class="layui-form-label"><span style="color: red;">*</span>电子邮箱</label>
                <div class="layui-input-block">
                    <input type="text" name="u_email" id="u_email"  onblur="checkEmail()"  value="<?php echo $admin['u_email']; ?>" lay-verify="required|email" placeholder="请输入电子邮箱" autocomplete="off" class="layui-input">
                </div>
            </div>
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                    <select name="u_c_id" lay-verify="required">
                        ...
                        <?php if(is_array($city) || $city instanceof \think\Collection || $city instanceof \think\Paginator): $i = 0; $__LIST__ = $city;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$depart): $mod = ($i % 2 );++$i;?>
                        <option value="<?php echo $depart['c_id']; ?>" <?php if($depart['c_id'] == $admin['u_c_id']): ?>selected<?php endif; ?>><?php echo $depart['c_name']; ?></option>
                ...
                <div class="layui-input-inline">
                    <select name="u_depart_id" lay-verify="required">
                        ...
                        <option value="<?php echo $depart['d_id']; ?>" <?php if($depart['d_id'] == $admin['u_depart_id']): ?>selected<?php endif; ?>><?php echo $depart['d_name']; ?></option>
                        <?php endforeach; endif; else: echo "" ;endif; ?>
                    </select>
                </div>
            </div>
            <div class="layui-form-item">
                ...
                <div class="layui-input-block">
                    <input type="text" name="u_job" value="<?php echo $admin['u_job']; ?>" lay-verify="required|title" placeholder="请输入职位" autocomplete="off" class="layui-input">
                </div>
            </div>
            <div class="layui-form-item">
                <div class="layui-input-block">
...
<script>
    ...
    function checkPhone(){
        ...
        $.ajax({
                ...
                dataType: 'json',
                data:{'u_phone':u_phone,'u_id':<?php echo $admin['u_id']; ?>},
            success:function (data) {
            console.log(data);
            if(data.code >1){
    ...
    function checkEmail(){
        ...
        $.ajax({
                ...
                dataType: 'json',
                data:{'u_email':u_email,'u_id':<?php echo $admin['u_id']; ?>},
            success:function (data) {
            console.log(data);
            if(data.code >1){
                layer.msg(data.msg, {icon: 2, time: 1000});
