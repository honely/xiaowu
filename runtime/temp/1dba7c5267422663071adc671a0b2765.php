<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:78:"G:\xampp\htdocs\bbb\public/../application/decoration\view\index\allocated.html";i:1544579067;}*/ ?>
<!DOCTYPE html>
<html>
...
<div class="mui-content">
    <div style="height: 44px;width: 100%">
        <div class="allocate-top">
            <a href="<?=url('index/decorating')?>"><div class="spans actives divs">装修中</div></a>
            <a><div class="spans divs">已转交（<?php echo $count; ?>）</div></a>
        ...
        <?php if(is_array($houses) || $houses instanceof \think\Collection || $houses instanceof \think\Paginator): $i = 0; $__LIST__ = $houses;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$hous): $mod = ($i % 2 );++$i;?>
            <div class="mui-card">
                <div class="mui-card-content">
                    <div class="mui-card-content-inner">
                        ...
                        <p><b>小区名称</b>：<?php echo $hous['h_building']; ?>
                            <span style="float: right;">房屋状态：<span style="margin-left: 8px;" class="mui-badge mui-badge-purple"><?php echo $hous['hd_status']; ?></span></span>
                        </p>
                        <p style="color: #333;">
                            <?php if($hous['is_paid_ratio'] == 1): ?>
                ...
                <div class="mui-card-footer">
                    <a class="mui-card-link" href="<?=url('index/details')?>?h_id=<?php echo $hous['h_b_id']; ?>&type=2">房源详情</a>
                    <a class="mui-card-link" href="<?=url('index/timeline')?>?h_id=<?php echo $hous['h_b_id']; ?>&type=2">装修进度</a>
                    <a class="mui-card-link" href="<?=url('index/dailylog')?>?h_id=<?php echo $hous['h_b_id']; ?>&type=2">监理记录</a>
                </div>
            </div>
...
<script>
    function formQuery(){
        ...
        location.href="<?=url('index/allocated')?>?&keywords="+keywords;
    }
</script>
<script>
                ...
                if(data.length<=0){
                    ...
                    for (var i=0;i<data.length;i++) {
                        ...
                        html+=' 签订日期：'+data[i].h_addtime+'<br/>'+
                            ...
                            '        </div>' +
                            '<div class="mui-card-footer">\n' +
                            '            <a class="mui-card-link" href="<?=url('index/details')?>?h_id='+data[i].h_b_id+'&type=2">房源详情</a>\n' +
                        '            <a class="mui-card-link" href="<?=url('index/timeline')?>?h_id='+data[i].h_b_id+'&type=2">装修进度</a>\n' +
                        '            <a class="mui-card-link" href="<?=url('index/dailylog')?>?h_id='+data[i].h_b_id+'&type=2">监理记录</a>\n' +
                        '        </div></div>';
                    }
                }
                $('#getMore').append(html);
